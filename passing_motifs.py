...
def normalize_motifs(counter, motif_list):
    """Returns values normalized to [0, 1] for a fixed list of motifs."""
    values = [counter.get(m, 0) for m in motif_list]
    total = sum(values)
    if total == 0:
...
def plot_motif_radar(match_ids_dict, team, zip_path,
                     save_path="passing_motifs_radar.png"):
    """
    Draws a radar chart of passing motifs.
    match_ids_dict: {"Home": [...ids...], "Away": [...ids...]}
                    or {"All matches": [...ids...]}
    """
    fig, ax = plt.subplots(
    ...
    motif_labels = MOTIFS_3
    N = len(motif_labels)
    angles = np.linspace(0, 2 * np.pi, N, endpoint=False).tolist()
    angles += angles[:1]

    colors = ["#e53517", "#42a5f5", "#43a047", "#ffca28"]

    for (label, match_ids), color in zip(match_ids_dict.items(), colors):
        sequences = extract_pass_sequences(match_ids, team, zip_path)
        counter = count_motifs(sequences, window=4)
        values = normalize_motifs(counter, motif_labels)
        values += values[:1]

        ax.plot(angles, values, color=color, lw=2, label=label, alpha=0.9)
        ax.fill(angles, values, color=color, alpha=0.15)

    ax.set_xticks(angles[:-1])
    ax.set_xticklabels(
        motif_labels, color="white", fontsize=13, fontweight="bold"
    )
    ax.set_yticklabels([])
    ...
    explanations = {
        "ABAB": "double triangle (players A and B alternate)",
        "ABAC": "central pivot - A passes to different partner",
        "ABCA": "3-player circuit returning to initiator",
        "ABCB": "player B is distribution pivot",
        "ABCD": "direct passes, 4 different players",
    }

    legend_text = "\n".join(f"{k}: {v}" for k, v in explanations.items())
    ax.set_title(
        f"{team} - passing motif profile",
    ...
    fig.text(
        ...
        bbox=dict(facecolor="#0d0d1a", alpha=0.7, edgecolor="none", pad=6)
    )
...
# def plot_top_motifs_bar_init(match_ids, team, zip_path,
#                         save_path="passing_motifs_bar.png", top_n=20):
#     """
#     Bar chart of the most frequent motifs (not just standard ABCD ones).
#     Useful for discovering unexpected passing patterns.
#     """
#     # sequences = extract_pass_sequences(match_ids, team, zip_path)
#     sequences = extract_pass_sequences_with_shots(match_ids, team, zip_path)
#     # counter = count_motifs(sequences, window=4, include_all=True)
#     # count across all window sizes
#     counter = Counter()
#     for seq in sequences:
...
#         "ABCD": "#ab47bc",
#     }
#     bar_colors = [palette.get(l, "#888888") for l in labels]
...
#     ax.set_title(
#         f"{team} - top {top_n} passing motifs (window of 4 players)",
...
# extended motif labels: standard + with shot
# MOTIFS_EXTENDED = [
#     "ABAB", "ABAC", "ABCA", "ABCB", "ABCD",
#     "ABABG", "ABACG", "ABCAG", "ABCBG", "ABCDG",
# ]
...
# def plot_player_motif_radar_init(match_ids, team, zip_path,
#                             top_n=5,
#                             save_dir="output/"):
#     """
#     Draws one radar chart per player (top N by minutes played),
#     showing both standard motifs and motifs ending in a shot (G).
#     Saves one PNG per player.
#     """
...
#         standard, goal = count_motifs_for_player(sequences, player, window=4)

#         # merge into single counter for extended motif list
#         combined = Counter()
#         combined.update(standard)
#         combined.update(goal)

#         N = len(MOTIFS_EXTENDED)
#         angles = np.linspace(0, 2 * np.pi, N, endpoint=False).tolist()
#         angles += angles[:1]

#         # normalize separately: standard and goal motifs
#         std_labels  = MOTIFS_EXTENDED[:5]
#         goal_labels = MOTIFS_EXTENDED[5:]

#         std_values  = normalize_motifs(standard, std_labels)
#         goal_values = normalize_motifs(goal, goal_labels)

#         # pad to full circle: alternate standard/goal or keep split
#         # use two separate polygons on same radar
#         all_values_std  = std_values  + [0] * 5 + std_values[:1]
#         all_values_goal = [0] * 5 + goal_values + [goal_values[0]]
...
#         # standard motifs - red
#         ax.plot(angles, all_values_std, color="#e53517", lw=2,
#                 label="standard motifs", alpha=0.9)
#         ax.fill(angles, all_values_std, color="#e53517", alpha=0.15)

#         # shot-ending motifs - gold
#         ax.plot(angles, all_values_goal, color="#ffca28", lw=2,
#                 label="motifs ending in shot", alpha=0.9)
#         ax.fill(angles, all_values_goal, color="#ffca28", alpha=0.15)

#         # axis labels
#         ax.set_xticks(angles[:-1])
#         ax.set_xticklabels(MOTIFS_EXTENDED, color="white", fontsize=10, fontweight="bold")
#         ax.set_yticklabels([])
#         ax.grid(color="white", alpha=0.15, linestyle="--")
#         ax.spines["polar"].set_color("white")
#         ax.spines["polar"].set_alpha(0.3)
...
# def plot_player_motif_radar_init_extended(match_ids, team, zip_path,
#                             top_n=5,
#                             save_dir="output/"):
#     """
#     Draws one radar chart per player (top N by minutes played),
#     showing standard motifs and shot-ending motifs across window sizes 2-4.
#     """
#     top_players = get_top_players_by_minutes(match_ids, team, zip_path, top_n=top_n)
#     sequences   = extract_pass_sequences_with_shots(match_ids, team, zip_path)
...
#     # split motifs into standard and goal
#     std_labels  = [m for m in MOTIFS_EXTENDED if not m.endswith("G")]
#     goal_labels = [m for m in MOTIFS_EXTENDED if m.endswith("G")]
...
#         # map to full circle: std on first half positions, goal on second half
#         # interleave on the 16-axis radar
#         all_values_std  = []
#         all_values_goal = []
#         std_idx  = 0
#         goal_idx = 0
...
#             if m.endswith("G"):
#                 all_values_std.append(0)
#                 all_values_goal.append(goal_values[goal_idx])
#                 goal_idx += 1
#             else:
#                 all_values_std.append(std_values[std_idx])
#                 all_values_goal.append(0)
#                 std_idx += 1

#         # close polygons
#         all_values_std  += all_values_std[:1]
#         all_values_goal += all_values_goal[:1]
...
if __name__ == "__main__":
    ...
    # radar chart: home vs away
    # plot_motif_radar(
    #     {"Home": home_ids, "Away": away_ids},
    #     TEAM_JSON, ZIP_PATH,
    #     OUT + "passing_motifs_radar.png"
    # )
